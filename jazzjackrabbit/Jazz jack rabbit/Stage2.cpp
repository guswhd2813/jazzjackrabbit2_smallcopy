...
{	
	...
	m_BitMap["stage2"] = (new CBitBmp)->LoadBmp(L"../Texture/map2.bmp");
	//서있는 동작
	...
	m_pBack = CFactory<CBack2>::CreateObj();
	...
	//m_ObjList[OBJ_MONSTER].push_back(CFactory<CLizard>::CreateObj(float(600),float(165)));
	...
	//m_ObjList[OBJ_MONSTER].push_back(CFactory<CLizard>::CreateObj(float(5000),float(5035)));	

	//m_ObjList[OBJ_MONSTER].push_back(CFactory<CTurtle>::CreateObj(float(2570),float(1445)));

	//m_ObjList[OBJ_MONSTER].push_back(CFactory<CTurtle>::CreateObj(float(5000),float(5555)));
...
int CStage2::Progress(void)
{
	...
	if( (m_ObjList[OBJ_PLAYER].back()->GetInfo().fX>=2900.f ))
	{
...
void CStage2::Render(HDC hdc)
{
	...
	m_pBack->Render(m_BitMap["back2"]->GetMemDC());
	...
	CRenderMgr::GetInst()->RenderObj(m_BitMap["back2"]->GetMemDC());
